...
export function createLeafletMap(mapContainer, defaultView = [46.3105761, 0.1725793], zoom = 12) {
  ...
  /**
   * @param {App.TerrainCombination[]} terrainsToDisplay
  ...
  function displayTerrains(terrainsToDisplay, selectedPolygonId) {
    ...
    terrains.forEach((terrainCombi) => {
      const isSelected = terrainCombi.id === selectedPolygonId
      terrainCombi.terrains.forEach((feature) => {
  ...
  function centerOnTerrain(terrainId) {
    var terrainCombi = terrains.find((f) => f.id === terrainId)
    if(!terrainCombi) {
      return
    }
    const coords = terrainCombi.terrains[0].geometry.coordinates[0]
      .map(([lng, lat]) => [Number(lat), Number(lng)])
